...
function render(){
  ...
  const templateAt=TEMPLATES.findIndex(t=>t.id===state.template);$('template-name').textContent=`${TEMPLATES[templateAt].name} · ${templateAt+1}/${TEMPLATES.length}`;
  document.querySelectorAll('.template-button').forEach(b=>{b.setAttribute('aria-pressed',String(b.dataset.template===state.template));b.disabled=state.exporting;});
